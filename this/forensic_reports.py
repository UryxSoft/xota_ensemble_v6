...
def _generate_executive_summary(report: ForensicReport) -> str:
    ...
    if report.stylometric_stats:
        ...
        if slv != -1 and slv < 15:
            ...
            # "Low burstiness" + "extremely high variance" reads as contradictory
            # to non-technical readers. Mathematically possible (CV=σ/μ can be
            # low when μ is very high, while σ² is enormous), but confusing.
            # Burstiness is the more meaningful AI-detection metric, so it wins.
            low_burst_already_reported = (burst != -1 and burst < 0.12)
            if not low_burst_already_reported:
                stylo_findings.append(
    ...
    # ══════════════════════════════════════════════════════════════════
    # 7b. PERPLEXITY ANALYSIS [NEW v3.7]
    # ══════════════════════════════════════════════════════════════════
    if report.perplexity_analysis:
        pa = report.perplexity_analysis
        pa_score = pa.get("ai_score", 0.0)
        pa_level = pa.get("risk_level", "")
        pa_tier  = pa.get("tier", "tier1")
        fv = pa.get("feature_values", {})
        ppl_mean = fv.get("proxy_perplexity_mean", 0.0)
        ent_mean = fv.get("token_entropy_mean", 0.0)
        lpr      = fv.get("low_perplexity_ratio", 0.0)
        valleys  = int(fv.get("perplexity_valley_count", 0))
        tier_label = "GPT-2 token-level" if pa_tier == "tier2" else "n-gram proxy (CPU)"

        if "HIGH" in pa_level:
            parts.append(
                f"PERPLEXITY ANALYSIS ({tier_label}): The text shows strong AI "
                f"perplexity signatures ({pa_score:.0%}). Mean proxy perplexity "
                f"is {ppl_mean:.1f} (AI-typical range), with {lpr:.0%} of text "
                f"windows showing AI-level predictability. The text's word patterns "
                f"are highly predictable to a language model, which is characteristic "
                f"of machine-generated content."
            )
        elif "MEDIUM" in pa_level:
            parts.append(
                f"PERPLEXITY ANALYSIS ({tier_label}): Moderate AI perplexity "
                f"indicators ({pa_score:.0%}). Mean proxy perplexity={ppl_mean:.1f}, "
                f"token entropy={ent_mean:.2f}. Compatible with AI-assisted writing, "
                f"heavily edited AI text, or structured human composition."
            )
        elif "INSUFFICIENT" in pa_level:
            parts.append(
                f"PERPLEXITY ANALYSIS: Insufficient text for reliable perplexity "
                f"analysis. At least 15 words are needed."
            )
        else:
            parts.append(
                f"PERPLEXITY ANALYSIS ({tier_label}): Low AI perplexity indicators "
                f"({pa_score:.0%}). The text shows natural unpredictability patterns "
                f"(mean proxy perplexity={ppl_mean:.1f}, entropy={ent_mean:.2f}) "
                f"consistent with human authorship."
            )

        if valleys > 1:
            parts.append(
                f"The perplexity curve shows {valleys} distinct low-perplexity "
                f"valleys, suggesting possible hybrid authorship — some sections "
                f"may be AI-generated while others appear human-written."
            )
...
<h2 class="section-header" onclick="toggleSection('perplexity-section')">
  Perplexity Analysis <span class="toggle-icon">&#9660;</span>
</h2>
<div id="perplexity-section" class="collapsible-section">
